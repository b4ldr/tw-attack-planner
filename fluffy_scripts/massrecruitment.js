function myMassRecruit(xmlConfig){
	...
	try {var arrLang=location.hostname.split('.');
		 ...
		 if(strLang=='ru')strVillage='Ãâ€ÃÂµÃ‘â‚¬ÃÂµÃÂ²ÃÂ½Ã‘ï¿½';
		}
	catch(objError){}
	...
	1.1		Added hard coded configuration (built at wartool.net/mass), minor performance enhancements
	...
	*/
	...
	var strImgDir='http://wartool.net/graphic/';
	if(window.frames.length>0){eleDoc=window.main.document;}
	eleDoc.getElementsByTagName('h2')[0].childNodes[0].nodeValue='Mass-Recruitment '+strVersion+'  (RIP ST)';
	var idx1;
...
/*		}*/
	...
	function myGetGroup(){
		try{return myInt(eleDoc.getElementById('menu_row2').cells[5].getElementsByTagName('a')[0].href.match(/\bgroup_id=(\d+)/)[1]);}
	...
	function myCreateCookie(idxGroup){
		...
		fnStatus("Config Value = "+strCookie);
		return true;
	}
	...
	function myGetCookie(idxGroup){
		fnStatus("Get config for group: "+strGroup);
		if (useCookies=="No"){
			if (ConfigVar[strGroup]){
				strCookie=ConfigVar[strGroup];
			}else{
			...
			strCookie=myReadCookie(idxGroup);
			}
	...
	function myGetConfigCookie(){
		var blConfig=true;
		idxGroup=myGetGroup();
		if(myGetCookie(idxGroup)){
		}else{
			myError('No configuration found for group: '+strGroup+'\nNavigate to wartool.com/mass and generate a new script that includes this this group. Then replace this script with the new one.');
			blConfig=false;
		}
	...
	function myConfigRow(){
		...
		function(arrQueueVals,idxQueue,arrParam){
			...
			myInput(objCell0,Math.floor(arrQueueVals[0]/(1000*60*60)),'arrInputQueue['+idxQueue+'][0]',3);
			myText(objCell0,' - ');
			myInput(objCell0,Math.floor(arrQueueVals[1]/(1000*60*60)),'arrInputQueue['+idxQueue+'][1]',3);
		});
		...
		function(arrUnit,idx1,arrParam){
			...
			myInput(objCell,0,'arrInputUnits['+idx1+']',5);
		});
...
/*		strMessage='Input changed:\n';
	...
	function myResource(objCell,idxResource,strMessage,intResource,blInput){
		...
		if($chk(blInput)&&blInput){
			myInput(objCell,intResource,'arrInputReserve['+idxResource+']',3);
			myText(objCell,' x 1000');
		}else{
	...
	function myFarm(objCell,strMessage,intFarmspace,blInput){
		...
		if($chk(blInput)&&blInput){
			myInput(objCell,intFarmspace,'arrInputReserve[3]',3);
		}else{
	...
	if(inputs[i].value=='Recruit') inputs[i].focus();
	}
