...
function get_travel_times(attackers, defenders, speed) {
	...
	for ( var d_count = 0; d_count < defenders.length; d_count++) {
		...
		travel_times[defenders[d_count]] = new Array()
		//a big number 
		for ( var a_count = 0; a_count < attackers.length; a_count++) {
			...
			travel_times[defenders[d_count]][attackers[a_count]] = distance * speed * 60;
		}
	}
	return travel_times	
}
...
function get_plan(travel_times, max_attack, type) {
	...
	for ( defend in travel_times) {
		if (typeof used_targets[defend] === 'undefined'  ) {
			used_targets[defend] = 0; 
		} 
		var used_attacker = new Array();
		for ( var count = 0 ; count < max_attack; count++) {
			var travel_time = "";
			var attacker = "";
			var fastest = 9999999999.0;
			console.log(count);
			for ( attack in travel_times[defend]) {
				if (typeof used_attacker[attack] === 'undefined'  ) {
					if ( travel_times[defend][attack] < fastest ) {
						attacker = attack;
						travel_time = travel_times[defend][attack];
						fastest = travel_time;
					}
				} 
			}
			if (  travel_time != "" ) {
				used_attacker[attack] = 0; 
				console.log(attacker);
				plan[attacker] = new Array();
				used_targets[defend] = used_targets[defend] + 1;
				//console.log(target);
				plan[attacker]['target'] = defend;
				plan[attacker]['travel_time'] = travel_time;
				plan[attacker]['type'] = type;
			}
		}
	}
	return plan	
